...
void writeDDSfile(char *filename,unsigned char *data,unsigned int bytes,unsigned int skip,unsigned int strip,int nofree)
   ...
   while (cnt++<bytes)
      ...
      if (cnt1+cnt2<(1<<DDS_RL) && (cnt1+cnt2)*imax(bits1,bits2)<cnt1*bits1+cnt2*bits2+DDS_RL+3)
         {
         cnt2+=cnt1;
...
unsigned char *readPVMvolume(char *filename,
   ...
   if (*components==2) volume=quantize(volume,*width,*height,*depth); //si el volumen es de 16 bits
